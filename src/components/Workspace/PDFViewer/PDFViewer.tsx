...
const Nav = ({ pageNumber, numPages, goToPrevPage, goToNextPage }: NavProps) => (
  <div className='flex h-9 w-full items-center justify-center bg-[#303030] text-white overflow-x-hidden rounded-t-lg shadow-md'>
    <div className="flex items-center space-x-4 ">
      <button onClick={goToPrevPage} disabled={pageNumber <= 1} className="flex items-center justify-center rounded-sm hover:bg-dark-fill-2 h-7 w-7 cursor-pointer">
      ...
      <button onClick={goToNextPage} disabled={pageNumber >= numPages} className="flex items-center justify-center rounded-sm hover:bg-dark-fill-2 h-7 w-7 cursor-pointer">
